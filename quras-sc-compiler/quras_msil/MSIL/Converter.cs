    ...
    public partial class ModuleConverter
    {
        ...
        private int ConvertCode(ILMethod method, OpCode src, NeoMethod to)
        {
            ...
            switch (src.code)
            {
                ...
                case CodeEx.Ret:
                    //return 在外面特殊处理了
                    _Insert1(Pure.VM.OpCode.RET, null, to);
                    break;
                case CodeEx.Pop:
                ...
                case CodeEx.Switch:
                    {
                        ...
                        //addrdata[0] = data[0];
                        //addrdata[1] = data[1];
                        //var code = _Convert1by1(Pure.VM.OpCode.SWITCH, src, to, addrdata);
                        //code.needfix = true;
                ...
                case CodeEx.Brtrue_S:
                    {
                        var code = _Convert1by1(Pure.VM.OpCode.JMPIF, src, to, new byte[] { 0, 0 });
                        code.needfix = true;
                        code.srcaddr = src.tokenAddr_Index;
                    }
                    break;
                ...
                case CodeEx.Brfalse_S:
                    {
                        var code = _Convert1by1(Pure.VM.OpCode.JMPIFNOT, src, to, new byte[] { 0, 0 });
                        code.needfix = true;
                        code.srcaddr = src.tokenAddr_Index;
                    }
                    break;
                ...
                case CodeEx.Bne_Un_S:
                    {
                        _Convert1by1(Pure.VM.OpCode.ABS, src, to);
                        _Convert1by1(Pure.VM.OpCode.SWAP, null, to);
                        _Convert1by1(Pure.VM.OpCode.ABS, null, to);
                        _Convert1by1(Pure.VM.OpCode.SWAP, null, to);
                ...
                case CodeEx.Throw:
                    {
                        _Convert1by1(Pure.VM.OpCode.THROW, src, to);//throw 会让vm 挂起
                        //不需要再插入return
                        //_Insert1(Pure.VM.OpCode.RET, "", to);
                    }
                    break;
                default:
#if WITHPDB
